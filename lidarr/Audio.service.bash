...
DownloadClientFreyr () {
	python3 /config/extended/ytdownload.py "$3" "$deezerAlbumTitle" "$lidarrAlbumTitle" "$lidarrArtistName" "$audioPath/incomplete" | tee -a "/config/logs/$logFileName"
...
DownloadProcess () {
	...
	do	
		...
		downloadCount=$(find "$audioPath"/incomplete/ -type f -regex ".*/.*\.\(flac\|m4a\|mp3\|opus\)" | wc -l)
